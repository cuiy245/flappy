(function() {
  ...
  var images = ['bg.png', 'ground.png', 'flappy_packer.png'].map(function(url) {
  ...
  var patternGround; // 路面画刷

  /**
   * 资源加载完成，开始游戏
   */
  function init() {
    //@see http://code.google.com/p/cakejs/source/browse/trunk/src/cake.js#28
  ...
  function renderGround() {
    var offset = status == 'playing' ? 
      -((new Date - statusTick) / 1200 * holdbackInfo.distance) % 840 : 0;
    context.save();
  ...
  function checkOver() {
    ...
    var tick = new Date - statusTick - holdbackInfo.beginTick;
    if (tick < 0) {
      return;
    }
